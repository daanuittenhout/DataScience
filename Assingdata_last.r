...
data[["nr_loans_MM"]] <- nr_loans_MM

#add income_MM to data
data[["income_MM"]] <- income_MM
...
data[["mortgagedummy"]] <- mortgageDummy

#second categorial variable
single <- married <- c(rep(999,length(marital_status)))
for (i in 1:length(marital_status)) {
  if(isTRUE(marital_status[i] == "married")) married[i] = 1 else married[i] = 0   #other is the reference category
  if(isTRUE(marital_status[i] == "single")) single[i] = 1 else single[i] = 0     #other is the reference category
}
#add marital_status Dummies to data
data[["single"]] <- single
...
amount_training <- round(nrow(data)*0.75) #checking how many observations the training set should contain.
amount_training

set.seed(100)
data$part<- runif(nrow(data), min=0, max =1)
Training <- data[data$part <= 0.75,]
Testing <- data[data$part > 0.75,]
...
meantest <- sumtest / ntest
meantrain <- sumtrain / ntrain
meantot <- (sumtest+sumtrain) / (ntest+ntrain)
...
TrainingKnn <- data.frame(Training[c(2,3, 8, 9,10)])
TestingKnn <- data.frame(Testing[c(2,3, 8, 9,10)])
cl <-unlist(Training[c(13)])
head(TrainingKnn)
head(TestingKnn)
head(cl)
#check lenngths are equal
length(cl)
length(TrainingKnn)
length(TestingKnn)
...
RMSE_k2 = rmse(Testing$riskDummy, predictk2)

RMSE_k3 = rmse(Testing$riskDummy, predictK3)
RMSE_k2
RMSE_k3
# better predictions for k = 3, because lower MSE
